...
async function rebuildViews(deploymentInfo) {
    ...
    try {
        ...
        for (const compDb of compDatabases) {
            ...
            console.log(`[REBUILDVIEW] Rebuilding views in ${dbName}`);
            rebuildViewsLog += `\n[REBUILDVIEW] Rebuilding views in ${dbName}`;

            // Connect to the comp database
            await conn.query(`USE ${dbName}`);

            await runViewSQL(dbName, deploymentInfo).catch((err) => {
                console.error('[REBUILDVIEW] Error running view file:', err?.message || err);
                rebuildViewsLog += '\n[REBUILDVIEW] Error running view file: ' + (err?.message || err);
